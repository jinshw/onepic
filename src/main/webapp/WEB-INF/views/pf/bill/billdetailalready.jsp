...
<script>
    ...
    if(hospitalId == '000001'){
        ...
        for (var i = 0; i < detailList.length; i++) {
            ...
            _billDetalList = _billDetalList +
                    ...
                    '<span class="bill-detail-num">';
             if(_obj.djZfzt != 2) {
             _billDetalList = _billDetalList +
             	 Math.round(numMulti(numMulti(_detailObj.mxDj, _detailObj.mxSl), 100)) / Math.pow(10, 2) + '</span>元';
             }
            _billDetalList = _billDetalList +
                    '</div>';
        }
    ...
    var flagRefund = "${flag}";
	console.log(flagRefund);
	var divRefund;
    if(flagRefund == "0") {
    } else {
    	divRefund = '<div class="weui_cell">'+
					'<a href="javascript:;" class="tf-bt" id="refund" data-flag="'+ flagRefund +'">申请退费</a>'+
        			'</div>';
    }
	$(document.body).append(divRefund);
	bindRefundClick();
</script>
</body>
</html>