...
<script src="${ctx}/static/js/billmanage.js"></script>
...
<script>
    ...
    if(hospitalId == '000001'){
        ...
        $("#total").html(_obj.djJe+"元");// 总金额

        var _billDetalList = "<p>账单明细</p>";
        var detailList = _obj.payInfoYZZXBeanList;
        var _detailObj;
        for (var i = 0; i < detailList.length; i++) {
            _detailObj = detailList[i];
            _billDetalList = _billDetalList +
                    '<div class="bill-detail-bar">' +
                    '<span class="bill-detail-name">' + _detailObj.mxMc + '</span>' +
                    '<span class="bill-detail-num">' + Math.round(numMulti(numMulti(_detailObj.mxDj, _detailObj.mxSl), 100)) / Math.pow(10, 2) + '</span>元' +
                    '</div>';
        }
        ...
        WeixinJSBridge.invoke(
                ...
                function (res) {
                    if (res.err_msg == "get_brand_wcpay_request:ok") {
                        $.toast("支付成功！", function () {
                            console.log('close');
                            window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx11f1b9aae05f3bd2&redirect_uri=http://www.yunju360.com/core/bill/initList&response_type=code&scope=snsapi_userinfo&state=park";
                        });
                    } else {
